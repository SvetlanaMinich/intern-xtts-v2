...
from TTS.Trainer.trainer.logging.tensorboard_logger import TensorboardLogger
...
class Overflow(BaseTTS):
    ...
    def on_init_start(self, trainer):
        ...
        if not os.path.isfile(TTS.Trainer.trainer.config.mel_statistics_parameter_path) or TTS.Trainer.trainer.config.force_generate_statistics:
            dataloader = TTS.Trainer.trainer.get_train_dataloader(
                training_assets=None, samples=TTS.Trainer.trainer.train_samples, verbose=False
            )
            print(
                f" | > Data parameters not found for: {TTS.Trainer.trainer.config.mel_statistics_parameter_path}. Computing mel normalization parameters..."
            )
            data_mean, data_std, init_transition_prob = OverflowUtils.get_data_parameters_for_flat_start(
                dataloader, TTS.Trainer.trainer.config.out_channels, TTS.Trainer.trainer.config.state_per_phone
            )
            print(
                f" | > Saving data parameters to: {TTS.Trainer.trainer.config.mel_statistics_parameter_path}: value: {data_mean, data_std, init_transition_prob}"
            )
            statistics = {
                "mean": data_mean.item(),
                "std": data_std.item(),
            ...
            torch.save(statistics, TTS.Trainer.trainer.config.mel_statistics_parameter_path)

        else:
            print(
                f" | > Data parameters found for: {TTS.Trainer.trainer.config.mel_statistics_parameter_path}. Loading mel normalization parameters..."
            )
            statistics = torch.load(TTS.Trainer.trainer.config.mel_statistics_parameter_path)
        ...
        TTS.Trainer.trainer.config.flat_start_params["transition_p"] = (
            init_transition_prob.item() if torch.is_tensor(init_transition_prob) else init_transition_prob
        )
        OverflowUtils.update_flat_start_transition(TTS.Trainer.trainer.model, init_transition_prob)
        TTS.Trainer.trainer.model.update_mean_std(statistics)
