...
CLIENT_ID_MAX_LEN = 4
BYTEORDER = 'little'
PORT = 8083            # 67.2.199.21:43398 -> 8083/tcp
HOST = '0.0.0.0'
...
async def handle_tts(ws:WebSocket, msg, 
    ...
    try:
        ...
        #     print(ex)
        client_id_b = client_id.to_bytes(4, BYTEORDER)
        ws.send(client_id_b+res_in_bytes)
        print(f"    > Result {len(res_in_bytes)} sent to client {client_id}")
        await clear_buf_files([f'cl {client_id} target_voice.wav'])
    except Exception as ex:
...
def run_server_tts():
    ...
    app.ws('/', {
            ...
            "idle_timeout": 60,
            "open": lambda ws, msg, opcode: asyncio.create_task(
